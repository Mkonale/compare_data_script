class Graphdb
  ...
  begin
    ...
    file = Roo::Spreadsheet.open('/mnt/c/users/Manoj/Downloads/PVM_UP/compare_tool_template_1.xlsx')
    file.sheet('Sheet1').each do |row|	
	 ...
	 display_order        = row[3].to_s.gsub(/ /, "").gsub(/\n/,"")
	 option_heading       = row[4].to_s.gsub(/'/, "").gsub(/\n/,"")
	 option_sub_heading   = row[5].to_s.gsub(/'/, "").gsub(/\n/,"")
	 option_description   = row[6].to_s.gsub(/'/, "").gsub(/\n/,"")
	 display              = row[7].to_s.gsub(/'/, "").gsub(/\n/,"")
		 ...
		 # @neo4j_session.query("match(n:ProductAttribute) where n.name='#{compare_name}' delete n")
	      ...
	      id_arr.each do |product_id|
	       ...
	       if @reg.match(prod_id)  #check regular expression
		    ...
		    p "Newly Created product_id is #{prod_id}"
		 else
			 p "Present product_id is #{prod_id}"
	         end
	       else
	...
	if  @reg.match(display_order)
	   prod_attribute_count =  @neo4j_session.query("match (n:ProductAttribute) where n.name='#{compare_name}' and n.display_order=#{display_order} and n.option_description='#{option_description}' return count(n)") 
		...
		p create_prod_attribute
		p "Created compare name"
	      else
		#p "Update attribute" 
